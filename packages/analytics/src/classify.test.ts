...
describe('classify()', () => {
  ...
  describe('confidence_score', () => {
    it('is 0 when alternation_rate sits exactly on the 0.08 threshold, regardless of sample size', () => {
      const result = classify({ failure_rate: 0.5, alternation_rate: 0.08, total_executions: 1000 });
      expect(result.confidence_score).toBeCloseTo(0, 5);
    });

    it('scales toward 1.0 as total_executions reaches TARGET_SAMPLE_SIZE (30) with a clear alternation signal', () => {
      const result = classify({ failure_rate: 0.5, alternation_rate: 0.5, total_executions: 30 });
      expect(result.confidence_score).toBeCloseTo(1.0, 5);
    });

    it('caps at 1.0 even when total_executions exceeds TARGET_SAMPLE_SIZE', () => {
      const result = classify({ failure_rate: 0.5, alternation_rate: 0.5, total_executions: 300 });
      expect(result.confidence_score).toBeLessThanOrEqual(1.0);
      expect(result.confidence_score).toBeCloseTo(1.0, 5);
    });

    it('is lower for a sample right at MIN_SAMPLE than for one at TARGET_SAMPLE_SIZE, same alternation_rate', () => {
    ...
    it('is 0 for INSUFFICIENT_DATA even with a large distance from the alternation threshold', () => {
      const result = classify({ failure_rate: 0.5, alternation_rate: 1.0, total_executions: 5 });
      expect(result.confidence_score).toBe(0);
    });
  });
});